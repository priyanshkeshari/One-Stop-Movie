...
async function fetchpm() {
    try{
        ...
        data.results.forEach(mediapm => {
            const pmCard = createpmCard(mediapm);
            console.log(pmCard);
...
function createpmCard(mediapm) {
    let {title, poster_path, vote_average, overview, original_title, id, release_date} = mediapm;
